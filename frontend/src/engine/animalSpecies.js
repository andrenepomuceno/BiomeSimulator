...
const DEFAULT_REPRODUCTION_TIMING = {
  mate_cooldown_multiplier: 1.35,
  gestation_ratio_from_cooldown: 0.8,
  min_gestation_ticks: 20,
};

function _mergeAnimalDefaults(simParams) {
  const baseMateCooldown = simParams.mate_cooldown ?? 60;
  const adjustedMateCooldown = Math.max(5, Math.round(baseMateCooldown * DEFAULT_REPRODUCTION_TIMING.mate_cooldown_multiplier));
  const gestationTicks = simParams.gestation_ticks
    ?? Math.max(
      DEFAULT_REPRODUCTION_TIMING.min_gestation_ticks,
      Math.round(adjustedMateCooldown * DEFAULT_REPRODUCTION_TIMING.gestation_ratio_from_cooldown),
    );

  return {
    ...simParams,
    mate_cooldown: adjustedMateCooldown,
    gestation_ticks: gestationTicks,
